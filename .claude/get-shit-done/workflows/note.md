...
- **Global scope**: `C:/Projetos/pomodoro-react/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Projetos/pomodoro-react/.claude/notes/`)
...
2. Glob `C:/Projetos/pomodoro-react/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Projetos/pomodoro-react/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Projetos/pomodoro-react/.claude/notes/` — works in any directory
